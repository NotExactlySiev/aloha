...
{
    Flaggy *temp_s1 = &D_80052060[channel];
...
int func_8001F64C(u32 id, s32 pan, s16 vol, s16 arg3, u16 arg4, s32 prio) {
    ...
    for (int i = 2; (i >= (prio & 0xFFFF)) && (i != 0); i--) {
        ...
        for (int j = 0; j < 24; j++) {
            if (D_80052060[j].unk6 != i) continue;
            u16 val = D_80052060[j].time;
            if (max < val) {
                max = val;
                max_index = j;
            }
...
found:
    ...
    return channel | D_80052060[channel].epoch;    
}
...
{
    return func_8001F9EC(handle, pan, D_80052060[handle & 0x1F].vol);
}

int func_8001F9A4(u32 handle, u16 vol)
{
    return func_8001F9EC(handle, D_80052060[handle & 0x1F].pan, vol);
}
...
{
    return D_80052060[handle & 0x1F].pan;
}

int func_8001FB58(uint handle)
{
    return D_80052060[handle & 0x1F].vol;
}
...
{
    return D_80052060[handle & 0x1F].active == 1;
}
