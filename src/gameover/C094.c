...
void announce_entering_main(void);
...
/* US:800ED354 JP:800ED254 */ s32 D_800ED354[7] = { 32, 33, 34, 35, 36, 37, 32 };
...
void func_800EB8F8(u32 buttons)
{
    ...
    case 3:
        ...
        if (D_800ED3E4 > 0)
            D_800ED3E4 -= 1;
        else if (screen_brightness > 0)
            screen_brightness -= 4;
        else
            stage = 4; // end this file
        break;
    }
...
void func_800EBF58(u32 buttons)
{
    ...
    if ((buttons & Pad1Up) && selected == 1) {
        selected = 0;
        play_effect(0x2c00);
    }

    if ((buttons & Pad1Down) && selected == 0) {
        selected = 1;
        play_effect(0x2c00);
    }
}
...
void render_background(void)
{
    ...
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++)
            sprite_render(0, 0x30, ((i << 7) - D_800ED394) - 0x40, ((j << 7) + D_800ED394) - 0x40, screen_brightness / 2, 2);
}
...
int main(void)
{
    ...
    if (world > 5)
        world = 5;
    jt.music_play(D_800ED354[world]);
